...
server.post("/topics/",function(req,res,next) {
  ...
  if(error) {
    ...
    req.body.slug=createSlug(req.body.title); // Generate a slug for new topics.
    next();
  }
});
...
function validateTopic(topic) {
  if(!topic.title) return "Title is required.";
  if(!topic.priorityLevel) return "Priority is required.";
  if(!topic.category) return "Category is required.";
  return "";
}
